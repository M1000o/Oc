...
import { Component, computed, DestroyRef, inject, signal, Output, EventEmitter } from '@angular/core';
...
@Component({
  ...
  imports: [CommonModule, ReactiveFormsModule, OnlyNumbers, OnlyLetters, AppToastHostComponent, MatIconModule],
  templateUrl: './proveedor-registration.html',
  styleUrl: './proveedor-registration.css',
})
export class ProveedorRegistrationModal {
  ...
  private showSuccessAndEmit(message: string): void {
    this.notificationService.success(message, ProveedorRegistrationModal.SUCCESS_FEEDBACK_MS);
    this.statusTimerId = window.setTimeout(() => {
      this.notificationService.dismiss();
      this.success.emit();
      this.closeRequest.emit();
    }, ProveedorRegistrationModal.SUCCESS_FEEDBACK_MS);
  }

  private showError(message: string): void {
    this.notificationService.error(message, ProveedorRegistrationModal.ERROR_FEEDBACK_MS);
    this.statusTimerId = window.setTimeout(() => {
      this.notificationService.dismiss();
    }, ProveedorRegistrationModal.ERROR_FEEDBACK_MS);
  }
